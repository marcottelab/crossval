module Statistics
  class Plot
    ...
    def initialize experiment_or_id, threshold = 0.0
      @experiment = experiment_or_id.is_a?(Fixnum) ? Experiment.find(experiment_or_id) : experiment_or_id
      @rocs       = @experiment.rocs.find(:all, :conditions => {:threshold => threshold}, :order => :auc)
      @rocs       = @experiment.calculate_extra_rocs(threshold).collect{ |r| Roc.new(r) } if @rocs.size == 0
    end

...
task :prepare_crossvalidation, :matrix_id, :stages, :n, :needs => :environment do |t,args|
  ...
  raise(ArgumentError, "Matrix id must be a Fixnum") unless matrix_id.is_a?(Fixnum)
  raise(ArgumentError, "Matrix #{matrix_id} is a mask and cannot be converted.") if matrix.is_a?(LeafMatrix)
  raise(ArgumentError, "Matrix #{matrix_id} is already a crossvalidation matrix. You must make a copy of it if you wish to prepare it for a different kind of crossvalidation.") if matrix.is_a?(NodeMatrix) && matrix.children.size > 0
  ...
  say_with_time "Changing matrix #{matrix_id} class to NodeMatrix" do
    Matrix.update_all( "type = 'NodeMatrix'", "id = #{matrix_id} AND type = 'Matrix'" )
    matrix.reload
